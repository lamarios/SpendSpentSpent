function ExpenseService($http) {
	...
	expenseService.add = function(newExpense, callBack) {
		...
		$http.post('/API/Expense', newExpense, HTTP_HEADERS).success(
				function(expense) {
	...
	expenseService.refresh = function(callback) {
		...
		console.log('Calling /API/Expense/ByDay?month='
				+ expenseService.selectedMonth);
		$http.get('/API/Expense/ByDay?month=' + expenseService.selectedMonth)
	...
	expenseService.refreshMonths = function() {
		while (expenseService.months.length > 0) {
			expenseService.months.pop();
		}

		console.log('Calling /API/Expense/GetMonths');
		// getting all the months available
		$http
				.get('/API/Expense/GetMonths')
				.success(
						function(data) {
							console.log(data);
							angular.forEach(data, function(value, index) {
								expenseService.months.push(value);
							});
