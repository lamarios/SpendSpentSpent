...
extension HouseholdService on Service {
  Future<Household> createHousehold() async {
    final response = await http.put(await formatUrl('$API_URL/Household'), headers: await headers);
  ...
  Future<Household?> getHousehold() async {
    final response = await http.get(await formatUrl('$API_URL/Household'), headers: await headers);
  ...
  Future<void> inviteUser(String email) async {
    final response = await http.get(
      await formatUrl('$API_URL/Household/invites/${Uri.encodeComponent(email)}'),
  ...
  Future<List<HouseholdMembers>> getInvitations() async {
    final response = await http.get(await formatUrl('$API_URL/Household/invites'), headers: await headers);

    processResponse(response);

    Iterable res = jsonDecode(response.body);

    return List<HouseholdMembers>.from(res.map((e) => HouseholdMembers.fromJson(e)));
  }

  Future<void> leaveHousehold() async {
    final response = await http.delete(await formatUrl('$API_URL/Household/leave'), headers: await headers);

    processResponse(response);
  }

  Future<void> deleteHousehold() async {
    final response = await http.delete(await formatUrl('$API_URL/Household'), headers: await headers);

    processResponse(response);
  }

  Future<void> setMemberAdmin({required String userId, required bool isAdmin}) async {
  ...
  Future<void> removeUserFromHousehold(String userId) async {
    final response = await http.delete(await formatUrl('$API_URL/Household/$userId'), headers: await headers);
