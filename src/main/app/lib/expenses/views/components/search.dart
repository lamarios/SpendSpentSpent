...
class Search extends StatelessWidget {
  ...
  Future<void> _setDates(BuildContext context) async {
    ...
    final dates = await showDateRangePicker(
      context: context,
      saveText: 'Search',
      firstDate: DateTime.fromMillisecondsSinceEpoch(cubit.state.searchParametersBounds.minDate ?? 0),
      lastDate: DateTime.fromMillisecondsSinceEpoch(cubit.state.searchParametersBounds.maxDate ?? 0),
      initialDateRange: DateTimeRange(
        start: DateTime.fromMillisecondsSinceEpoch(cubit.state.searchParameters.minDate ?? 0),
        end: DateTime.fromMillisecondsSinceEpoch(cubit.state.searchParameters.maxDate ?? 0),
      ),
                  ...
                  children: [
                    Expanded(
                      child: AnimatedCrossFade(
                        ...
                        crossFadeState: state.searchParametersBounds.categories.isEmpty
                        ...
                        secondChild: SizedBox(
                          ...
                          child: ListView(
                            ...
                            children: state.searchParametersBounds.categories.map((c) {
                              var isSelected =
                                  state.searchParameters.categories.length == 1 &&
                                  state.searchParameters.categories[0].id == c.id;

                              var categoryColor = context.read<HouseholdCubit>().state.getCategoryColor(context, c);
                                  ...
                                  child: Padding(
                                    ...
                                    child: AnimatedContainer(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        color: isSelected ? categoryColor.primaryContainer : lightBackground,
                                      ),
                                      duration: panelTransition,
                                      child: Center(
                                        child: getIcon(
                                          c.icon ?? "",
                                          color: isSelected
                                              ? categoryColor.onPrimaryContainer
                    ...
                    Expanded(
                      child: RangeSlider(
                        ...
                        divisions: state.searchParametersBounds.maxAmount > 0
                            ? state.searchParametersBounds.maxAmount - state.searchParametersBounds.minAmount
                            : null,
                  ...
                  children: [
                    Expanded(
                      child: FilledButton.tonalIcon(
                        ...
                        label: Text('${_searchFormat.format(from)} - ${_searchFormat.format(to)}'),
                      ...
                      onPressed: () async {
                        ...
                        if (context.mounted) {
                          ...
                          final params = ShareParams(
                            ...
                            files: [XFile.fromData(file, mimeType: "text/csv", name: fileName)],
                          );
