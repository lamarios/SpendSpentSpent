...
class OneExpense extends StatelessWidget {
  ...
  const OneExpense({super.key, this.showExpense, required this.expense, this.showIcons = true, this.showDate = false});
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, color: expenseColors.primaryContainer),
            child: Padding(
              padding: const EdgeInsets.all(12.0),
              child: getIcon(expense.category.icon!, size: 20, color: expenseColors.onPrimaryContainer),
            ),
          ),
          Expanded(
            child: Column(
              ...
              children: [
                Visibility(
                  ...
                  child: Padding(
                    ...
                    child: Text(expense.note ?? '', maxLines: 1, overflow: TextOverflow.ellipsis),
                  ),
                ),
                if (showDate)
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 8.0),
                    child: Text(
                      _formatter.format(DateTime.fromMillisecondsSinceEpoch(expense.timestamp)),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Visibility(
                        ...
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 8.0),
                          child: Icon(Icons.refresh, color: colors.onSurface.withValues(alpha: 0.5), size: 15),
                        ),
                      ),
                    ],
                  ),
                  ...
                  if (expense.files.isNotEmpty)
                    ConditionalWrapper(
                      ...
                      wrapper: (child) => InkWell(
                        onTap: () {
                          AutoRouter.of(context).push(ImageViewerRoute(images: expense.files, initiallySelected: 0));
                        },
                        child: child,
                      ),
                      child: Stack(
                        ...
                        children: [
                          ExpenseImage(file: expense.files.first, width: 50, height: 50, showStatus: false),
